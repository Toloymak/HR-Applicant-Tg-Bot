...
public class CancelRevokeCallbackHandler : ICallbackHandler<CancelRevokeCallback>
{
    private readonly ITelegramBotClient _tg;
    private readonly BotUserService _botUserService;
    private readonly ApplicationStatusService _statusService;
    private readonly ILogger<CancelRevokeCallbackHandler> _logger;

    public CancelRevokeCallbackHandler(
        ITelegramBotClient tg,
        BotUserService botUserService,
        ApplicationStatusService statusService,
        ILogger<CancelRevokeCallbackHandler> logger)
    {
        _tg = tg;
        ...
        CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Error handling CancelRevokeCallback");
            
            await _tg.SendMessage(
                chatId: chatId,
                text: "❌ An error occurred while cancelling the revocation. Please try again.",
                cancellationToken: ct
            );
        }
    }
